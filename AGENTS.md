...
- **既存の 顧客・案件・キャラを 流用する 前に、「どのステージで 初めて 出るか」を 確かめる。**
  まだ **先の ステージ**の ものは 使わない。先に 出すと **後の 教材の 初登場を 壊す**
  （2026-09-10: 朝礼の 案件に Aoba抹茶カフェ／そら書店を 当てて 却下された。設計01 §2.1
  「新しい 世界を 作らず、この 世界に 新しい 場面を 足す」を、**順番を 見ずに 流用の 許可と 読んだ**のが 原因。
  使って よいのは **そのステージより 前に 出て いる もの だけ**。無ければ **完全オリジナルを 作る**）。
- **ネガティブな 例を、名前の ある 登場人物に やらせない。** 松井社長を のぞく 登場人物は
  ...
  **主人公（学習者）から 見て、名前の ある 登場人物は みんな 先輩**である（2026-09-10）。
  「同期」「同僚」に しない。
