...
1. **結果をぼかさない。** 正解か不正解か、合格か不合格かが学習者の理解に要る場面では、
   それを**はっきり明記する**。ぼかして「よくできたね」のような綺麗事だけを出すのは
   **害でしかない**（2026-09-03 の指定）。判定を言ったあとに次の行動を1つ添える。
   - **会話練習（ミーティング・たいわ）は別の型。** 相手役のAIはポジティブに受け取る。
     ただし**分からなかったときは分かったふりをせず聞き返す**。褒めて流すと、
     通じていないことが学習者に最後まで伝わらない（同日の指定）。
   - 禁じるのは**人に向けた見下し**だけ。禁止語の一覧は `FORBIDDEN_LEARNER_WORDS`
     （`src/content/schema.ts`）で、いまは「ダメです」「ダメだ」の2語。
     `lint:content` が**コンテンツと src の文字列を両方**走査する。
   - **2026-09-03 に外したもの**: 「不正解」「間違いです」「間違いだ」（判定語。禁じると
     判定そのものをぼかす方へ逃げる）と、「〜できなくても だいじょうぶ」「〜でなくて OK」等の
     7語（「もともとそんな指示してない」）。
...
10. **順路と 枠組みを 勝手に 作らない・崩さない。**（2026-09-10。機械検査は 無い＝文章だけが 頼り。
    支えは ルーブリック R9〜R11 と PRテンプレの 2つ）

- **移植は 差分ゼロが 既定。** 旧アプリから 移す ときは、足すのも 削るのも **その場で 理由を 書く**。
  `node scripts/diff_port.mjs <旧URL> -- <新JSON>` を 回して PR に 貼る。
  とくに **因果の 接続詞（ですから・だから）と 英語の 添え（Fact / Boss / Error）を 落とさない**。
  「読みやすく した つもり」で **意図が 壊れる**——2026-09-10 の 実例では
  旧の 問いが 消えて 答えが 1行目に 移り、「なぜ 藤木さんに 報告するのか」が 丸ごと 落ちて いた。
- **仕組みを 外す ときは、その 仕組みが 担って いた 学習上の 役割を 先に 書き出し、
  引き継ぎ先を 決めてから 外す。** パスワードを 外す＝答えを 隠さない、では ない
  （パスワードは「考える 時間を 強制する 装置」でも あった）。
  「ステージの 関門に まかせる」は 引き継ぎ先では ない——関門は 教材と 教材の あいだにしか 無い。
- **学習者に 見せる 枠組み（型・部品・パターンの 数と 名前）を、本文に 無い ところで 新しく 作らない。**
  ミニゲームの ために 分類軸を 発明しない。要るなら 先に 本文へ 足し、**絵の 中の ことばとも そろえる**。
  **1ステージに 枠組みは 1つ。**
- **教材を 足す／並べ替える ときは、ステージの 並びを before → after で 書いてから 着手する。**
  並びは「足した順」では なく「前提が 揃う順」で 決める。
- **設問は ヒントを 閉じた まま 答えられる こと。** ヒントは 足場であって 前提では ない。
  産出（自由記述・ミーティング）の 前には **場面と 役を 必ず 渡す**。
- **ネガティブな 例を、名前の ある 登場人物に やらせない。** 松井社長を のぞく 登場人物は
  **実在の ネクストメイクの 社員・役員が モデル**（名前を 少し 変えて ある）。
  できない 例・失敗する 例は、**登場人物では ない 架空の 新人**（山田さん・田中さん など
  よく ある 名前）に 担わせる。※「日本語が 拙い＝能力が 低い」描写の 禁止（01ガイド
  アンチパターン1）は 相手が だれでも そのまま 効く。
