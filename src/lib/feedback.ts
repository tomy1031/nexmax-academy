...
export const FEEDBACK = {
   ...
   *
   * **あとの 会話が この もんだいを こたえノートに 出す ときだけ** 使う
   *（`QuizRunner` の `inNotebook`）。そうで ない ときは `quiz.written`。
   */
  "quiz.prepared": {
    tone: "praise",
    title: "じゅんび できました！",
  ...
  /**
   * 正解の 無い もんだいを 出し終えたが、**書いた ものを 開く 会話が 無い**とき。
   *
   * 「会話の じゅんび」とは 言わない——つぎは スキットや 答え合わせの ページで、
   * そこに「📋 自分の こたえ」は 無い。言えるのは 書いた ものが **この 画面の 下に
   * 並んで いる** ことと、いちばん 下の ボタンで 先へ 進む ことだけ。書けた 数は
   * 大見出しが もう 言って いる。
   *
   * 「下の ボタン」だけだと、すぐ 下の「書き直す」を 指して しまう。**いちばん 下**
   * （枠の「つぎは ▶」／最後の 教材なら「ステージを おえる」／枠の 外なら
   * 「もんだいを えらぶ」）と 言えば、どの 置き場でも 正しい。札の 名前で 言わない
   * のは、置き場ごとに 札が ちがう ため。
   *
   * 色と 絵は `info`（🤖）。`hint` の 💡 は 同じ 画面の 設問の ヒント札
   * （「💡 考える ヒント」）と 同じ しるしで、「これも ヒント？」と 読めて しまう。
   */
  "quiz.written": {
    tone: "info",
    title: "書いた ことは 下で もう一度 読めます",
    next: "読んだら、いちばん 下の ボタンで つぎへ 進みましょう",
  },
  ...
  /**
   * 話題は 合って いるが、**聞いた 相手が 担当では ない**とき（3人の たいわ）。
   * 「だれが 知って いるか」を 考えるのが 練習なので、札は 開けず、相手を かえる 道を 示す。
   * だれが くわしいかは 画面が 教材の 名前で 添える（ここは 名前を 持てない）。
   */
  "talk.askOwner": {
    tone: "hint",
    title: "その ことは、ほかの 相手が くわしいよ",
    next: "🎤 で 話しかける 相手を かえて、もう一度 聞こう",
  },
  "talk.notReady": {
    tone: "info",
    title: "じゅんびちゅう",
