...
const VERIFIED_SPLIT_COMPOUNDS: ReadonlySet<string> = new Set([
  ...
  "注文管理機能",
  /*
   * 「商品一覧」「商品一覧機能」— 商品(しょうひん)＋一覧(いちらん)＋機能(きのう) で
   * 組み立ての 読みも 正しい。2026-09-14 に 目で 確認（朝礼の 記事の 練習の 例文）。
   */
  "商品一覧",
  "商品一覧機能",
  /*
   * 「一覧機能」—「メニュー一覧機能」の 漢字の ところ。一覧(いちらん)＋機能(きのう) で
   * 組み立ての 読みも 正しい。2026-09-23 に 目で 確認（朝礼の 記事の 練習の 例文）。
   */
  "一覧機能",
  "動作確認用",
  "学生自身",
  ...
  "年間日本語",
  /*
   * 送りがなで 終わる 語（確認待ち・回答待ち・見送り）。読み辞書には
   * ["確認待ち","かくにんまち"] と 送りがな込みで 入って いて 画面は 正しいが、
   * ここが 見るのは **漢字の かたまりだけ**（確認待）なので、送りがなを 含む
   * 見出しは 永久に 当たらない。組み立ての 読み（かくにんま＋ち）も 正しい。
   * 2026-09-09 に 目で 確認。
   */
  "確認待",
  "回答待",
  ...
  "見込",
  /*
   * 「直書き」— 読み辞書は ["直書き","じかがき"] で 画面は 正しく、解析も
   * 直[ジカ]＋書き[ガキ] と 同じ 読み。ここが 見るのは 漢字の かたまり（直書）だけ
   * なので 割れて 見える。2026-09-11 に 目で 確認（朝礼「自分の 画面の 直書きを
   * きょうの 昼までに 表に 移して いただけませんか」）。
   */
  "直書",
  /*
  ...
  "藤木取締役",
  /*
   * 「KhmerSabaiの 朝礼」ページ（asakai_team）の 語。どれも **熟語＋熟語**で、
   * 割れて 読まれても 組み立ての 読みは そのまま 正しく、むしろ 2語に 分かれた
   * ほうが 学習者は 読みやすい（注文確認・注文管理 と 同じ 立場）。
   * 2026-09-16 に 目で 確認。
   */
  "注文確認画面",
  "決済後",
  ...
  "間連絡",
  /*
   * 「読み込み中表示」— 読み辞書は ["読み込み中","よみこみちゅう"] で 画面は
   * 読み込み中[よみこみちゅう]表示[ひょうじ] と 正しい。ここが 見るのは 漢字の
   * かたまり（中表示）だけ なので、かな を またぐ 見出しは 当たらず、
   * 中[なか]＋表示[ひょうじ] に 割れて 見える。2026-09-16 に 目で 確認
   *（夕礼の 作業記録「読み込み中表示を 追加」）。
   */
  "中表示",
]);
