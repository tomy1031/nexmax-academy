 ...
 *
 * `lint:content` が 見るのは **教材データ（JSON）だけ**で、コードが 組み立てる
 * 文字列は どの 種別を 足しても 通らない（クエストの 教材そのものは 2026-09-11 に
 * `collectLabeledTexts` へ 入った が、ここの 文は その 外側に ある）。だから
 ...
 * **ログが 自分で 書く ことば**の 読み。
 *
 * 教材の 読み辞書と 重ねて 使う（`mergeFuriganaEntries(これ, quest.furigana)`）。
 * **同じ 表記が ぶつかったら 教材が 勝つ**——教材の 読みは `lint:content` が
 * 見張って いるが、こちらは 見張られて いない から。`["上が","あが"]` は
 * 教材の `["上","うえ"]` より 長いので、重ね順に よらず 最長一致で 先に 当たる
 * ——「確認した上で」は うえで の まま。
 *
