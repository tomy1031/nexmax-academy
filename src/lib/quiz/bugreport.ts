...
export function buildBugReviewPrompt(context: BugReviewContext, kanjiRetry = false): string {
  ...
  const lines: string[] = [
    ...
    "- **言い方が お手本と ちがっても、意味が 同じなら 満点**。お手本の ことばに 言いかえさせる ヒントは 書かない" +
      "（2026-09-25 の 指定「もう少し 柔軟に」）。",
    "- 画面の 部品や 動きの 呼び方は いろいろ ある。どれでも 同じと 見る" +
      "（れい: お知らせ＝モーダル＝ポップアップ＝ウィンドウ／押す＝クリック＝タップ／" +
      "表示される＝出る＝開く＝見える／送信＝送る）。",
    "- 25点に しないのは、**バグの 中身が ちがう・足りない**ときだけ。",
