...
export const judgeOutputSchema = z.object({
  ...
  /**
   * できたことを1つ。**中身のない ほめことばは 書かせない**。
   *
   * 2026-09-10「がんばったね、などの無駄な表現も省いてください」で **空を 許した**。
   * 名指しできる ことが 無い とき（何も 話して いない・形しか 見て いない）に
   * 無理に 埋めると、「だいじょうぶです」「いい 答えです」の ような
   * **中身の 無い 1行**が 必ず 出る。空なら 画面が 欄ごと 消す。
   */
  praise: z.string(),
...
export const JUDGE_RESPONSE_SCHEMA = {
  ...
  /*
   * **`praise` は 要らない**（2026-09-10 の 指定「がんばったね、などの 無駄な 表現も
   * 省いてください」）。
   *
   * 必ず 書かせて いた ころ、名指しできる ことが 何も 無い 回でも 何かを 書く しか
   * なく、中身の 無い ほめことばに なって いた。スキーマから 外して、
   * **名指しできる ことが ある ときだけ** 書かせる。
   * 受け口（`judgeResponseSchema`）は 空を 通し、画面は 空なら 欄ごと 出さない。
   */
  required: ["language", "relevance", "form", "reply", "fix", "exampleAnswer", "retry", "glossary"],
} as const;
...
export function buildJudgePrompt(context: JudgeContext, kanaRetry = false): string {
  const lines = [
    ...
    "- praise: **名指しできる ことが ある ときだけ** 1つ。ぐたいてきに。",
    "  **無ければ 空の まま**に します（欄ごと 出しません）。中身の 無い ほめことば",
    "  （「がんばりました」「いいですね」「よく できました」）は 書きません——",
    "  ほめる ために 何かを さがす と、言って いない ことを ほめる ことに なります。",
