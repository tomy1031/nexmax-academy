...
/** 名前として 埋める ところ（`◯◯さん` だけ。むきだしの `◯◯` は 埋めない）。 */
const CALL_NAME = /◯◯さん/gu;
/** 名前が 無い ときに 落とす 呼びかけ（あとに 続く 読点も 一緒に 落とす）。 */
const CALL_NAME_DROP = /◯◯さん[、，,]?[ 　]*/gu;

/**
 * **呼びかけの 名前だけ**を 差し込む（朝礼・夕礼の 司会の ことば）。
 *
 * ## なぜ `fillName` を そのまま 使えないか
 * `fillName` は `◯◯` を **ぜんぶ** 名前に する。ところが 朝礼・夕礼では
 * 同じ `◯◯` が **2つの 意味**で 使われて いる:
 *
 * - `では 次に ◯◯さん、お願いします。` … 学習者の **名前**（10か所）
 * - `「きのうは ◯◯を しました」の 形で` … 学習者が **埋める 空欄**（38か所）
 *
 * そのまま 当てると、ヒントが「きのうは ソピアを しました」に なる。
 * だから **`◯◯さん` と 書いて ある ところだけ**を 名前と 見る。
 * 空欄の 目印は アプリ ぜんたいで `◯◯` の まま（教材ごとに 記号を 変えない）。
 *
 * 名前が まだ 無い 端末（デモ・診断まえ）では **呼びかけごと 落とす**——
 * 「あなたさん」に なる のを 避ける ため（`fillName` と 同じ 考え方）。
 */
export function fillCallName(text: string, learnerName: string): string {
  const name = learnerName.trim();
  if (name !== "") return text.replaceAll(CALL_NAME, `${name}さん`);
  return text
    .replace(CALL_NAME_DROP, "")
    .replace(/[ 　]{2,}/gu, " ")
    .trim();
}
