 ...
 *
 * ## なぜ 問いごとに 持つか
 * 「採点の ものさしは 答える 前に 見せる。観点は しつもんごとに 持つ」
 *（2026-08-31 の 指定・docs/constraints.md）。ぜんぶ 同じ 表で 見ると、
 * 聞いて いない ことで 減点され、学習者は 何を 直せば よいか 分からなく なる。
 *
 * ## `model`（模範解答）は AIにも 学習者にも 渡す
 * AIには **見本**として 渡す（ずれた 直しを させない ため）。学習者には
 * **書いた あと**に 出す（2026-09-20 の 指定「模範解答の 表示だけでなく、
 * 入力された 文章を ベースにした ブラッシュアップ回答も 見やすく」）。
 * 書く 前に 出さない のは、写すだけの 問いに しない ため——だから 画面の
 * ボタンは **1文字も 書いて いない あいだは 押せない**（`ai-review-panel.tsx`）。
 */
const aiReviewSchema = z.object({
  /** 見る ところ（答える 前に 画面に 出る）。 */
  checks: z
    .array(
      z.object({
        id: z.string().regex(/^[a-z0-9_-]+$/),
...
const asakaiPanelSchema = z
  .object({
    ...
    label: plainText,
    /**
     * ことばでは なく **形**で 見る。いまの 使い道は **進みぐあいの 札**だけで、
     * 当たるのは 割合の ことば（`75%`・`75パーセント`）か「進捗」＋数
     *（`saysProgress`・`src/lib/meeting/panels.ts`）。
     *
     * 「数字が 1つ でも あれば」で 見て いた ころ、`09:00`（作業記録の 行頭）や
     *「上位6スキル」「検索結果0件」で **パーセントを 言わない まま 開いて いた**。
     */
...
/**
 * どこまで できたかの 1行。名前は 字で、状態は 印と ことばで 出す。
 *
 * 16字まで 許すのは、**名前を 短く すると 何の しごとか 分からなく なる**から
 *（「テスト 20こ」と「テスト 8こ」が 並ぶと どちらの テストか 読めない →
 *「ログインの テスト 20こ」「パスワードの テスト 8こ」）。画面は 2行まで 折り返す。
 */
const asakaiProgressSchema = z.object({
  /**
   * しごとの 名前。28字まで——**その日で 中身が 変わる しごと**が ある
   *（「決済APIと つなぐ（ABA）」→ 社長の 要望の あと「（ABA/ACLEDA）」。
   * 2026-09-16 の 指定）。何に つなぐのかが 名前から 消えると、
   * 増えた ことも 戻った ことも 画面から 読めない。
   */
  label: plainText.max(28),
  /**
   * やる ことが 絵で 分かる 印（絵文字1〜2字）。字を 読む 前に あたりが つく。
   * **清書の 絵（`image`）が 無い ときの 逃げ道**として 残す——絵は 10枚 いちどに
   * 出るので、1枚でも 届いて いない 行が あると そこだけ 空の 四角に なる。
   */
  icon: plainText.max(4).optional(),
  /**
   * 清書の 絵（願い #441・2026-09-16 の 指定「各タスクに やる ことが わかる
   * わかりやすい 画像を つけて ください」）。台帳は `scripts/images/asakai_tasks.json`。
   *
   * **`src` だけを 持つ**。プロンプト全文は 台帳が 正で、ここへは 写さない——
   * この 表は 5つの 場面が それぞれ 10行 持つ ので、プロンプトを 焼くと
   * 同じ 文が 50回 並び、作りおき（RSCの props）が 数百KB 太る。
   */
  image: imageSlotSchema.optional(),
  state: z.enum(["done", "now", "later"]),
  /**
   * **その日 増えた しごと**（翌日には 外す）。2026-09-16 の 指定
   *「追加した ことが わかる ように、翌日だけ『追加』アイコンを つけます」。
   *
   * 表は 毎日 同じ 並びなので、黙って 1行 増えても 気づけない。
   * 増えた 日だけ 印を 出して、**社長の 要望が しごとに なった**ことを 見せる。
   */
...
const asakaiCardSchema = z.object({
  ...
  /**
   * 金曜日までの しごとを **ぜんぶ** 並べる（2026-09-15 の 指定
   *「5つだけでは なく 全て 表形式で 並べ、終わった ものは チェックを 入れて」）。
   *
   * 前は `length(5)` だった。担当表（原本）の しごとは 10ある ので、5つに まとめた
   * 時点で **どれが 終わったのかを 画面から 言えなく なって** いた——報告で
   *「きのう したこと」を 言う ときに 見る 表なのに、その もとが 無かった。
   */
  progress: z.array(asakaiProgressSchema).min(1).max(20),
...
const asakaiSceneSchema = z
  .object({
    ...
    /**
     * **その 日だけ 足す 見かた**（AIへの 言い渡し・2026-09-14 の 指定
     *「ステージ（曜日）ごとに プロンプトは 変更できると いいと 思います」）。
     *
     * 教材ぜんたいの `judgePrompt` に **継ぎ足す**。5日ぶんの 写しには しない——
     * 写しで 持つと 片方だけ 直り、どちらが 正なのかが データから 読めなく なる
     *（ポップアップの 殻を 1つに まとめた ときと 同じ 理由）。
     * ここに 書くのは **その 日に しか 当てはまらない こと**だけ
     *（水曜の「夕礼を 待たずに その場で 報告した ことを 高く 見る」・
     * 金曜の「明日では なく 来週」）。
     *
     * 鍵が 無い 端末では 使われない——ことばの 照合だけで 動き、**開く 条件は
     * 1つも 変わらない**（`src/lib/meeting/asakai-judge.ts` の 覚え書き）。
     */
    judgeNote: plainText.optional(),
  })
