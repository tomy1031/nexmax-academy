 ...
 *
 * ## 名前は任意（2026-09-07 の指定）
 * `name` が無いエリアは、地図に**札を出さない**。絵と一言だけの土地になる。
 * 名前を必須にすると「何か書かないと保存できない」ので、埋めるためだけの名前が並ぶ。
 */
export interface MapArea {
  id: string;
  /** 画面に出す景色の名前。**国名を入れない**。空なら地図に札を出さない */
  name?: string;
  /** 表示名のよみ（ルビに使う）。名前があるときだけ意味を持つ */
  reading?: string;
  /** 背景画像。`public/` からのパス */
  image: string;
