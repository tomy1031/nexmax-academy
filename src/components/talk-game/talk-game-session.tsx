 ...
 *
 * **外へ 出して いる**のは 読みの 見張り（`tests/talkgame_chrome_furigana.test.ts`）の ため。
 * `lint:content` は src の 文字列を **禁止語しか** 見ないので、画面の 飾りの 読みが
 * まちがって いても 機械は 止めない——ここだけは テストで 押さえる。
 */
export const CHROME_ENTRIES: readonly (readonly [string, string])[] = [
  ["社長", "しゃちょう"],
  ["会社", "かいしゃ"],
  /*
   * **送りがなで 読みが 変わる**（2026-09-03 の 指摘「これまでの話 は はな ではなく はなし」）。
   * 1字の「話」だけを 置くと、名詞の「これまでの 話」まで はな と 読ませて しまう。
   * 動詞の 形を 先に 置き（最長一致）、裸の「話」は 名詞の はなし に する——
   * 教材の 読み辞書（`content/meetings/kaisha_matsui.json`）と 同じ 並べ方。
   */
  ["話し", "はなし"],
  ["話す", "はなす"],
  ["話そ", "はなそ"],
