 ...
 *
 * ## 色の 面を 作る ときの 決まり（2026-09-21 の 指定で 明文化）
 * **文字の 色と ふりがなの 色を、いつも 2つ セットで 決める。**
 * いちばん かんたんなのは `text-white`（Tailwind の クラス）で 文字色を 置く こと——
 * `globals.css` に `.text-white rt { color: currentColor }` が ある ので、
 * ふりがなも いっしょに 白に なる。
 *
 * **`style={{ color: "#fff" }}` と インラインで 書くと ふりがなは ついて こない。**
 * クラスの 付いて いない 要素なので 上の 規則に 当たらず、ふりがなだけ 濃い灰の まま
 * 残る（2026-09-21 に 答え合わせの 緑の チップで 実発生。2026-08-18・08-21 と 同じ 事故で
 * **3回目**）。だから インラインの 白は `tests/ruby_on_color.test.ts` が 機械で 弾く。
 * 白 以外の 濃い 地（紺・桃・金）に のせる ときも 同じ——`text-white` か
 * この `RUBY_ON_COLOR` を 足して、ふりがなの 色を 決めない ままに しない。
 */
