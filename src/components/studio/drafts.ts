...
const DEFAULT_MEETING_PERSONA = [
  ...
  /*
   * **条件つきの 指示を 書かない**（2026-08-20）。
   *
   * ここに「学生が だまったら、答え方の れいを 1つ 見せて ください。」と
   * 書いて あった ため、相手が それを **台本の ト書きとして 読み上げた**
   *（学習者に「(学生がだまったら:例えば…)」と 届いた）。
   * やり方の 説明は 相手に 渡さない。合図が 要る 場面は アプリが 見て、
   * **言う ことばそのもの**を その場で 渡す。
   *
   * しつもんも アプリが 出す ので、相手には させない。
   */
  "しつもんは 画面が します。あなたは 自分から しつもんを しないで ください。",
  "学生に 向けて 話す ことばだけを 言います。やり方の 説明は 声に 出しません。",
