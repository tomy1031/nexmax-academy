 ...
 *   2. **タブを 隠した とき**（授業の チャイムで 別の 画面へ 行っても 消えない ように）
 *   3. 開いた 直後に 1回（前の 授業で 送れなかった ぶん・デモモードで ためた ぶん）
 *   4. 送りきれなかった ぶんが 残って いたら、もう一度
 *
 * `pagehide`（タブを 閉じる）でも 呼ぶが、**間に合う とは 限らない**——閉じたあとに
 * 通信は 走らないし、写す 中身は `import()` で 取りに 行く ので なおさら である。
 * それでも 損は しない: 端末には 記録が 残り、印は 送れた ときにしか 付かないので、
 * 次に 開いた ときに そのまま 流れる。当てに するのは 2 の ほう。
 */
  ...
  useEffect(() => {
    ...
    const flush = () => {
      ...
      void import("@/lib/records/sync")
        .then(async ({ flushRecords }) => {
          const { more } = await flushRecords();
          /*
           * 送りきれて いなければ もう一度 予約する。残る 道は 3つ——1回の 上限で
           * 切った／別の 流しと 重なって 見送った／送れなかった。ここで 予約し直さないと、
           * 「残りは 次の 流しで 入る」は **次に 端末の 記録が 変わるまで 起きない**
           * （何も 学ばずに 画面を 閉じた 学習者の ぶんが、そのまま 残る）。
           */
          if (more) schedule();
        })
        .catch(() => {
