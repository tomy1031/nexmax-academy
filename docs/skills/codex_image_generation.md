...
**継ぎ目の契約**: 各タイルは 1024×1536 の縦長で、**四辺すべてが海**。上端10%と下端10%の帯は
平らな海色 `#2E9FD6`（RGB 46,159,214）のみにする。実際の継ぎ目は `CloudBand` の雲海が覆うので
端の色は厳密でなくてよいが、雲からはみ出す事故を避けるためこの契約は守る。

| # | 保存先 | 表示名 | 主題（差分プロンプト） |
|---|---|---|---|
| 1 | `public/img/scenes/area1_cambodia.webp` | いしの みやこ | アンコールワット風の五塔石造寺院＋熱帯雨林・砂糖椰子・高床の家 |
| 2 | `public/img/scenes/area3_vietnam.webp` | うみの いわやま | ハロン湾風の石灰岩の岩山群＋ジャンク船・棚田・提灯の村 |
| 3 | `public/img/scenes/area5_taiwan.webp` | かいだんの まち | 九份風の階段街＋赤提灯・茶畑の段々・霧の山 |
| 4 | `public/img/scenes/area_misty_peaks.webp` | きりの やまなみ | 水墨画風の霧の岩峰＋瓦屋根の城下町・多層の塔・石橋・茶畑 |
| 5 | `public/img/scenes/area_palace_town.webp` | みやこの まち | 瓦屋根の宮殿と旧市街＋紅葉・露店、奥に現代のビル群 |
| 6 | `public/img/scenes/japan_goal.webp` | 日本（ゴール） | 富士山・桜・東京のスカイライン・鳥居（**横長 1536×1024**。地図の最下部の帯） |

- 6枚目だけは役割が違う（縦に積むタイルではなくゴール帯）ので、横長のまま据え置く。
