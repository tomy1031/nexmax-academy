...
学習者ページの `revalidate` を **300秒 → 7日（604800）**にした（18ファイル）。
デプロイの 間（本番は 火・水・金、STG は 1日8回）は 一度も 古くならないので、
輪に 入らない。`s-maxage` が 大きく なるぶん Cloudflare の 拠点キャッシュが 効き、
**そもそも Worker が 起きない**。教材は **デプロイのたびに 作り直される**ので、
学習者が 見るのは 常に その日の 授業前に 出した 中身に なる。
...
**Workers 有料（$5/月）に すると CPU は 10ms → 30秒**（最大5分まで 上げられる）。
フルSSR 570ms は 上限の 2% に なり、**この 種の 事故は 消える**。
そのときは `revalidate` を 300 に 戻してよいし、STG も KV モードへ 戻せる。
§0.7 の 前任者の 結論（「有料に すれば 事実上 消える」）が 正しかった。
