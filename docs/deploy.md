...
`/wordtest` は セット 10本（213KB）を まるごと `ArcadeGame` に 渡して いた。

| 作りおき | 前 | 後 |
| --- | --- | --- |
| `wordtest` | 1101 KB | **49 KB** |
| `wordtest/stage11_haizoku` | 1103 KB | **46 KB** |

一覧の 行（見出し・語数）は 13KB しか ないので **サーバで 描いたまま**。
押す ものは すぐ 出て、重い 中身だけが あとから 追いつく。
ステージから 来た ときは これまでどおり **その ステージの セットだけ**を 渡す
（取りに 行かせると、二段目の えらぶ 画面に よその 課の ことばが 並ぶ）。

#### 覚えて おく こと —— **サーバ部品→クライアント部品の 受け渡しは 5〜7倍に なる**

渡した ものは **HTML と RSC の 両方**に 積まれ、文字列として 逃がされる ぶん ふくらむ。

| 渡した データ | 作りおき |
| --- | --- |
| 辞書 250 KB | 1.5〜1.7 MB |
| 単語セット 213 KB | 1.1 MB |

作りおきに **当たって いても** その 大きさが CPU に なる。**100KB を 超える データを
props で 渡さない**。渡したく なったら `src/lib/json-asset-store.ts`
（`public/` に 置いて ブラウザに 取りに 行かせる）を 使う。

### 0.15 cron を 5本に しても 直らなかった —— 時計を GitHub の 外へ 出した（2026-09-22）

**症状**: 「授業の 前に 自動で 出る」が **一度も 時間通りに 動いて いなかった**。
毎回 人が 手で 出して いた。

**調べた こと**（`gh api repos/tomy1031/nexmax-academy/actions/runs?event=schedule`）。
§0.11 と 同じ 調べかただが、今回は **46本 ぜんぶ**を 測った。

| イベント   | ラン作成までの 時間（2026-09-01〜09-18）                  |
| ---------- | --------------------------------------------------------- |
| `push`     | **3秒**（直近20本 すべて）                                |
| `schedule` | **3時間15分〜4時間43分**（46本の 中央値 3時間45分・最大 11時間） |

cron は 16:12〜17:12 ICT に 5本。実際に ランが 作られるのは **20:30〜21:55 ICT**。
授業（17:30）の **3時間あと**である。9/15・9/16・9/18 の 手動 dispatch の 時刻が
17:21 / 17:23 / 16:53 ICT なのは、その 尻拭いを 人が して いた ため。

**§0.11 の 対策は 外れて いた。** 「5本 ぜんぶが 落ちない かぎり 間に合う」と 書いたが、
実際には **5本 とも 同じだけ 遅れて いた**。5本は 別々の 待ち行列では なく、
**同じ 1つの 遅れ**を 5回 見て いただけ だった。数を 増やしても 直らない 種類の 故障。

**時差でも ない。** `12 9 UTC` = `16:12 ICT` は 正しい。遅れ幅が 日ごとに 88分 ぶれるので
固定の ずれでも なく、**時刻を 早めても 直らない**——早すぎる 日が 出て、直前に 入れた
教材が 載らなく なる ほうへ 化けるだけ。

**効いた 見かた**: `push` は 3秒で 走って いる。**詰まって いるのは Actions では なく
`schedule` の 配達だけ**。ならば **イベントさえ 外から 投げれば 時間通り**である。

**直した かた**: 時計を GitHub の 外へ 出した。詳細と 入れかたは
[`workers/deploy-kicker/README.md`](../workers/deploy-kicker/README.md)。

- `deploy.yml` に `workflow_dispatch` の **`target: auto`** を 足した。
  これは **cron と まったく 同じ 道**（`env.AUTO` が `schedule` と `auto` を 同じに 扱う）。
  既存の `staging` / `production` / `push` / `schedule` の 動きは 変えて いない。
- 起こし役を 2つ 外に 置いた。どちらも 16:10 / 16:52 / 17:02 ICT。
  - **Cloudflare の Cron Trigger**（`workers/deploy-kicker`）… 本命。機械に 依らない。
    GitHub の 細粒度 PAT（Actions: Read and write・このリポジトリのみ）を Worker の
    secret に 1つ 置く。**期限切れが 唯一の 黙る 原因**なので カレンダーに 入れる。
  - **開発機の launchd**（`workers/deploy-kicker/mac/`）… 保険。鍵ゼロ（`gh` の ログインを 使う）。
    その Mac が 起きて いる ときだけ 効く。
- **GitHub の cron は 消さない。** 外の 時計が 2つ とも 黙った ときの 最後の 受け皿。
  遅れて 届いても、すでに 出て いれば「出す必要が あるか 決める」が 空振りに する。

**3つ 起こしても 本番へ 出るのは 必要な ときだけ 1回**——この 性質は §0.11 で 作った
`scripts/lib/should_deploy.mjs`（本番の `/api/version` を 見る）が そのまま 担う。
起こし役を 増やしても KV の 枠（1000件/日）に 響かないのは この ため。

**教訓**: 「冗長化した」と 書く 前に、**冗長に なって いるかを 測る**。
同じ 仕組みを 5つ 並べても、故障が その 仕組み自体なら 5つ とも 同時に 黙る。

### 0.16 GitHub が デプロイを 止める ——「怪しい ワークフロー」の 足止め（2026-09-22）

**症状**: 2026-09-22 10:09 UTC から、「デプロイ」ワークフローの ランが **ぜんぶ
`action_required`** に なった。ジョブは 1つも 起動せず、実行ページに こう 出る:

> GitHub detected that this workflow file may be malicious. It will not run until
> someone with write access approves it.

STG も 本番も 出なく なり、人が ブラウザで 1本ずつ 承認するまで 動かない。
2026-08-26 にも 同じ ことが 起きて いる。

**正体**: GitHub が 2026-07-28 に 入れた 機能
（<https://github.blog/changelog/2026-07-28-github-actions-holds-potentially-malicious-workflows-for-approval/>）。
性質が 4つ あり、どれも こちらでは 変えられない:

- **public リポジトリだけ**に 効く（このリポは public）
- **切る 設定が 無い**。GitHub が 自動で かける
- 止まるのは **ラン単位**。1本 承認しても、次の push は また 止まる
- 承認には **ブラウザの ログインセッション**が 要る。**PAT では 押せない**
  （API は 403）——だから `workers/deploy-kicker`（§0.15）にも 肩代わりできない

**何を 怪しいと 見たかは GitHub が 公表して いない。** ただ、状況証拠は 揃って いた:

- `deploy.yml` を 直した コミット（`60d661b`・09:55 UTC）の **14分後**から 止まり始めた
- 同じ リポジトリの `CI` と `教材の 音声づくり` は **一度も 止まって いない**
- `deploy.yml` にだけ、検査器が 定番で 叩く 形が 2つ あった

**やったこと（2026-09-23）**: その 2つを 落とした。機能は 落として いない。

1. `SECRET_NAMES: ${{ toJSON(secrets) }}` を 消した。**Secret を まとめて 1つの
   環境変数へ 流し込む 書き方**は、持ち出しの 典型そのもの。用途は
   「鍵が 足りない とき 登録済みの 名前を 一覧する」だけ だったので、
   **こちらが 読む 名前を 直に 3つ 書く**形に 替えた。
2. `run:` の 中の `${{ github.event.inputs.target }}` を `env:` 経由（`$TARGET`）に した。
   **式の 中身が シェルの 一部に なる 形**は、どの 検査器も 叩く。

**どちらも 戻さない。** とくに `toJSON(secrets)` は、親切のつもりで また 書きたく
なる——書いた 瞬間に 授業前の デプロイが 止まる 可能性が ある。

**これで 直ると 保証は できない**（基準が 非公開の ため）。止まり続ける なら、
次の 手は **デプロイを GitHub Actions の 外へ 出す**こと（Cloudflare 側の ビルド）。
GitHub が 止めるのは GitHub の ランだけ なので、それなら 構造的に 効く。
