...
#### STG の作りおきは静的アセット側（2026-09-02 に移した）

STG のデプロイは **KV へ1件も書かない**。統合ブランチ運用で STG の更新が頻繁に
なるため、1回 約110件 × 1日 8.2回 ＝ 900件/日で無料枠（書き込み 1000件/日）を
食い潰し、**その日の本番が作りおきゼロで出る**——という 2026-08-26 の事故を
構造から断つ。

**2026-08-27 の「KV のまま温めない」は失敗だった。** 各ページ初回のフルSSRで
後追いに温まる前提だったが、**重いページはその初回が Error 1102 で落ちるので
永久に温まらない**（§0.12）。そこで置き場そのものを静的アセットへ移した
（`scripts/preview_alias.mjs` の `usesAssetsCache`）。ブランチ確認URLが前から
使っている仕組みで、KV書き込みは 0件のまま**全ページぶんが必ず載る**。

- 代償は **先生の直し（DB）が STG に出るのが「次のデプロイ時」になる**こと
  （それまでは 300秒）。STG は `integration` へマージするたび上がる（1日8回ペース）
  ので、遅れは数時間に収まる。**管理画面（`/admin/*`）は `force-dynamic` で DB を
  直に読むので、スタジオ側の表示は常に真**——ずれるのは学習者向けページだけ。
- **`scripts/loadcheck.mjs` は STG に向けてよくなった**（作りおきに当たるため）。
- **本番は従来どおり KV を全ページ温める**（`cf:deploy` が中で `populateCache` を呼ぶ）。
...
単語ゲーム（単語テスト）を授業で 20人が同時に開いたとき、Cloudflare の
...
- CPU **10ms/リクエスト**（Cloudflare の公称）・メモリ **128MB**（超えると Error 1102）。
...
> **2026-09-02 の訂正** — この 10ms は 実測と 合わない（本番で 665ms が 通った）。
> ただし 「冷えた フルSSR が 引き金」という この節の 見立て 自体は 正しかった。
> 効いている 上限が どれかは §0.12 を 見る。
...
##### 訂正: 「居座る」のは **その ラン だけ**（2026-08-29 に 実測）

上の「**いちど 掛かると 居座る**」は 言い過ぎだった。正しくは:

- **承認は ラン 1本ごと**。1本 承認しても、**同時に 止まって いた 別の ラン は
  止まった まま**（人が それぞれ 押すか、放って おくしか ない）。
- ただし **承認を 1回 済ませた あとに 作られた ラン は、そのまま 動く**。
  「以後 ずっと 全部 止まる」わけでは ない。

観測の 並び（2026-08-29・cron を 5本に した 日）:

| 時刻(UTC) | ラン | きっかけ | 結果 |
| --- | --- | --- | --- |
| 10:41 | `33248397867` | push `1d666cf`（deploy.yml を **触った**） | **承認待ち・ジョブ0件** |
| 11:19 | `33249852802` | push `5275a65`（触って いない） | **承認待ち・ジョブ0件** |
| 11:20 | `33248739586` | 上の 10:41 を **人が 承認**（`run_attempt: 2`） | 動いた・成功 |
| 11:21 | `33249941554` | dispatch（承認の あと） | 動いた・成功（attempt 1） |
| 11:59 | `33251389131` | dispatch（承認の あと） | **動いた（attempt 1・承認 要らず）** |

11:19 の ラン は いまも 承認待ちの まま だが、11:21 と 11:59 は 何も 押さずに
動いて いる。**門は 閉じて いない。**

だから **cron も、承認を 1回 済ませた あとなら そのまま 動く**
（2026-08-28 の schedule ラン `33211372267` も 8/27 の 承認の あとで 動いている）。

**運用の 規則は 変わらない**——むしろ はっきりする:

> **授業日（火・水・金）の 前に `.github/workflows/` を 触らない。**
> 触ったら、**その 日の うちに デプロイを 1回 通して**（人が 1回 承認して）
> 帰る。承認が 1回 済んで いれば、次の cron は 止まらない。

**確かめかた**（AI でも できる。承認は できないが、門が 開いて いるかは 見られる）:
`workflow_dispatch` で staging を 1本 出し（`ref=integration`。STG は KV を
1件も 使わない ので 何回 出しても ただ）、その ラン が `action_required` に ならず
`run_attempt: 1` の まま ジョブへ 進めば、門は 開いて いる。
...
### 0.12 1102 の 犯人は CPU 時間では なかった（2026-09-02 に 実測）

STG で Error 1102（Worker exceeded resource limits）が 出た。ログの CPU TIME は
**255,057µs = 255ms**。ところが 同じ日に 本番を `wrangler tail --format json` で
見ながら 測ると、こうだった。

| 叩いたもの | cpuTime | outcome |
| --- | --- | --- |
| `/`（作りおきに 当たる） | 4〜11ms | ok |
| `/api/health/content`（`force-dynamic`・全 kind を zod 検証） | **507 / 534 / 658 / 665ms** | **ok** |

**665ms が 通って 255ms が 落ちる**——ここから「効いている 上限は CPU 時間では ない」と
結論したが、**これは 誤りだった**（§0.13 で 訂正）。無料枠は バーストを 見逃すので、
成功した リクエストの CPU が 上限を 超えて いても 上限の 反証に ならない。再現も できる——重い 公開エンドポイントへ
`curl` を 12本 同時に 投げると、STG で 12本中 1本が `error code: 1102` を 返した
（本番でも 冷えた isolate で 500 が 3本）。

#### なぜ STG から 出たか

STG は デプロイ時に KV の 作りおきを **温めない**（§0.7・`shouldPopulateRemoteCache`）。
コメントは 「各ページ初回だけ フルSSRで、そのあと 自然に 温まります」と 書いていたが、
**重いページでは そうならない**。初回の フルSSR が 落ちる → 作りおきが 書かれない →
次も 落ちる、の 堂々めぐりに なる。KV の 実測が それを 示していた。

| | 鍵の数 | populateCache 由来 |
| --- | --- | --- |
| 本番の buildId | 132 | 110 |
| STG の buildId | **13** | **0** |

STG の 13鍵は デプロイから 2時間 経っても 増えなかった。

#### 決め手は「作りおきに 当たるか」だった

同じ時刻に 同時40本ずつ 投げると、こう 分かれた。

| 叩いた先 | 結果 |
| --- | --- |
| STG `/`（作りおきに **当たる**） | **40/40 成功** |
| 本番 `/`（作りおきに **当たる**） | **40/40 成功** |
| STG `/api/health/content`（作りおき **無し**） | 31〜34/40（残りが 1102） |

**当たれば 40本 同時でも びくともせず、外すページだけが 落ちる。**
実行時の zod を やめた ぶん 1リクエストは 軽くなったが、それだけでは 1102 は
消えなかった。**STG の 作りおきを 載せる**のが 答えで、KV では 枠が 足りないので
静的アセット側へ 移した（§0.7・`usesAssetsCache`）。

#### 温め直しでは 直らない（数えた）

STG の デプロイは **平均 8.2回/日**（`gh run list --workflow=deploy.yml`。直近は
11・15・7・6・15・11回）。1回の 温めが 約110件 書くので **8.2 × 110 ≒ 900件/日**。
KV の 無料枠は 書き 1000件/日 なので、ここに 本番の 温め（110件）と 後追いの
書き込み（約150件）が 乗ると 枠を 超え、**その日の 本番が 作りおきゼロで 出る**。
2026-08-27 に STG の 温めを 外した 判断は 正しい。間違っていたのは
「そのあと 自然に 温まる」という 見込みの ほうだった。

#### 直したこと — 検証を ビルド時に 寄せた

`src/lib/content.ts` は git 由来の 教材を **毎リクエスト zod に かけ直して** いた。
同じ `contentSchema` は `lint:content` が pre-commit と CI で 通しているので、
Worker の 上で 通し直しても 分かることは 1つも 増えない。

| | 手元の Mac |
| --- | --- |
| 全 kind の `contentSchema.safeParse` | 33.9ms |
| 通さず そのまま 使う | **0.11ms** |

しかも zod は 検証の たびに **全オブジェクトの 複製**を 作るので、生と 検証ずみを
二重に 抱えて メモリを 食う——効いている 上限が メモリだった 以上、ここが 直撃する。

そこで **`scripts/lib/bake_content.ts` が ビルド時に 一度だけ zod を 通し、その出力を
`src/content/git-contents.generated.ts` へ 焼き込む**。実行時は そのまま 使う。
焼くのは 必ず `parsed.data` の ほう——`contentSchema` には `.default()` が 57か所 あり、
生 JSON を 焼くと 既定値が 消えて 画面が 静かに 変わる。ずれの 見張りは 2つ:
`lint:content` の 焼き込み比較と、`tests/git_contents_baked.test.ts`
（「もう一度 通しても 変わらない」＝ 既定値まで 入っている ことの 検査）。

**KV の 書き込みは 1件も 増えない。** 冷えた ページの 初回 SSR が 通るように なるので、
「そのあと 自然に 温まる」が 本当に なる、という 直しかたである。

### 0.13 上限は やはり CPU 10ms —— 作り直しの 輪が 閉じない（2026-09-02 に 確定）

§0.12 で「CPU では ない」と 書いたのは **誤り**。授業中の 本番を
`wrangler tail --format json` で 見ながら 学習者に 開いてもらって 確定した。

```
outcome=ok           cpu=567ms   /kaisha
outcome=ok           cpu=283ms   /kaisha/article-kaisha_matsui_junbi
outcome=exceededCpu  cpu=10ms    Error: Worker exceeded CPU time limit.
outcome=exceededCpu  cpu=10ms    Error: Worker exceeded CPU time limit.
log: ['Revalidation failed for /kaisha/link with status 503']
```

**無料枠は バーストを 見逃すが、使い切ると そこから 全部 きっかり 10ms で 切られる。**
だから「成功した リクエストが 10ms を 超えている」は 上限の 反証に ならない。
1102 を 見たら **`outcome` と `exceptions` を 見る**（数字だけ 見ない）。
本番は tail できる。プレビュー版（STG）は できない。

#### 輪が 閉じない

作りおきが「古い」に なると、OpenNext の 横取りは リクエストの 中で
`await queue.send()` し、memoryQueue が 自分自身へ HEAD を 投げて **まるごと フルSSR**
する。フルSSR は 実測 280〜570ms で、10ms の 30〜60倍。落ちる。落ちると
**鮮度が 更新されない**ので、次の リクエストも また 作り直そうとする——
**輪が 閉じないまま、人数ぶん 積み上がる**。

#### 直し ①: 作り直しの 経路に 入らない（`force-static`）

学習者ページの `revalidate` を **消した**（18ファイル・`export const dynamic = "force-static"`）。

はじめは **300秒 → 7日（604800）** に 逃がしたが、これは「7日 以内に かならず
デプロイが ある」ときしか 効かない。自動デプロイは **中身が 同じなら 出さない**
（`scripts/lib/should_deploy.mjs`）ので、連休で 1週間 止まれば **7日目に 全ページが
いっせいに 古く なって 輪が 再開する**。Worker が KV に 書いた 分も 7日で 期限切れに
なる（`src/lib/cache/kv-expiring-cache.ts`）。時間で 逃げるのを やめて、**古い判定
そのものを 起こさない**。

`s-maxage` が 大きく なるぶん Cloudflare の 拠点キャッシュも 効く。教材は
**デプロイのたびに 作り直される**ので、学習者が 見るのは 常に その日の 授業前に
出した 中身に なる。見張りは `tests/learner_pages_static.test.ts`
（`src/app` の どこかに `revalidate` が 生えたら 落ちる）。

**代償**: 先生が スタジオで 直した 内容（DB）が 出るのが「300秒後」から
「次の デプロイ」に なる。実行時に 追いつかせる 直しは **ブラウザ側で 読む** かたちで
別に 進める（§0.14）。

#### 直し ②: 当たった ときの 荷物を 減らす（§0.14）

作りおきに **当たって いても** 1リクエスト 50〜137ms かかって いた。原因は
**作りおき 1件が 1.5MB** あった こと。§0.14 を 見る。

#### 上限そのものを 上げる 道

Workers 有料（$5/月）に すると CPU は 10ms → 30秒 に なる。**ただし これは
