...
1. 画面で 1回 **Approve and run** を 押す。
2. **`.github/` を 触らない コミット**を もう 1つ main に 入れる。ランは
   新しい HEAD で 作られるので、そこからは 承認が 要らなく なる
   （この 節を 足した コミットが まさに それ）。

**授業の 前に ワークフローを 直さない。** 直すなら、そのあと **必ず デプロイを
1回 通して**（承認を 済ませて）から 終わること。
