...
### 0.6 本番リリースの輪 — STG確認 → 「本番OK」 → 本番反映（忘れない仕組み）

ユーザーはクライアント。**全ての編集は STG で確認してもらい、「本番OK」が出たものだけ本番へ出す。**
仕組みとしては main→STG は自動（deploy.yml）・本番は手動、と分かれていたのに、
**「OKを求める → OKが出たら出す」の記憶がチャットにしか無く**、セッションが変わると
確認待ちの存在ごと忘れられていた（2026-08-18 実測: STG は1日に14回更新されたのに
本番リリースは2回。#91〜#93 が本番待ちのまま放置されていた）。

輪を閉じる決まり（それぞれ持ち場が違う。全部そろって初めて忘れなくなる）:

1. **見える化** — `npm run handoff` が「■ 本番待ち」欄に、本番に出ていない main の
   コミット一覧を毎回出す（本番 `/api/version` の SHA と origin/main の差分。どのセッションでも
   開始時に必ず目に入る）。
2. **確認依頼** — main へ統合して STG が変わったら、報告は
   「URL＋操作＋見るポイント＋**OKの返しかた**」の4点で出す。OKの返しかたは
   **「本番OK」と一言返すだけ**、に固定する（何をすればリリースされるのかを迷わせない）。
3. **記録** — チャットは流れて消えるので、**マイタスクボードに「STG確認 → 本番OK」タスクを
   1件だけ**保ち、確認待ちの中身（PR番号と一言）をそこへ追記していく（スキル
   `managing-my-tasks`。統合のたびに新しいタスクを乱立させない。本番へ出したら閉じる）。
4. **実行** — ユーザーの「本番OK」（チャットでもタスク上でもよい）を見つけたセッションが、
   その場で本番へ出す。鍵は GitHub 側にあるので AI がそのまま実行できる:

   ```bash
   gh workflow run deploy.yml -f target=production
   ```

   `gh run watch` で完走を見届け、本番 `https://academy.nexmax.workers.dev/api/version` の
   `sha` が origin/main と一致したことを確かめてから、タスクを閉じて報告する。
5. **OKの単位** — STG は常に main 全体なので、「本番OK」は**その時点の main のスナップショット
   全体**への OK。OK のあとに新しい統合が入ってから出す場合は、増えたぶんを一覧にして
   報告に含める（黙って余計なものを乗せない）。
