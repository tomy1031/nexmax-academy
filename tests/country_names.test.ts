...
describe("すでに使ってよいと決まっている地名は 通す", () => {
  it("日本（学習の目的地そのもの）", () => {
    expect(checkCountryNames("f", saying("日本で はたらきます。", [["日本", "にほん"]]))).toEqual(
      [],
    );
  });

  it("カンボジア（学習者自身の国。すでに公開ずみの教材にも入っている）", () => {
    expect(checkCountryNames("f", saying("カンボジアから 来ました。"))).toEqual([]);
  });
});

describe("あたらしい国名は 止めずに 知らせる", () => {
  /*
   * 規律9 は「新しい国名を出すときは事前にユーザーへ確認する」であって
   * 禁止ではない。error にすると、先生は確認を取る前に検査を外す方へ動く。
   */
  it("ベトナムは warn（確認の合図）", () => {
