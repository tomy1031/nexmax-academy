 ...
 *
 * 見るのは 4つ:
 *  - ステージトップに セットが **ぜんぶ 行で ならぶ**（初級①…上級）
...
test("ステージトップに セットが ぜんぶ ならぶ", async ({ page }) => {
  ...
  await expect(cards.first()).toHaveAttribute("href", "/arcade/stage23_kaisha");
  await expect(cards.last()).toHaveAttribute("href", "/arcade/stage23_kaisha_jokyu");

  /*
   * 札の 字は **セット名＋その セットの 見出し**（同じ 名前が 6行 ならばない）。
   * 見出しには ルビが 合成される ので、字の 間に よみが 入る——
   * `toContainText` で 通しの 字を 見ずに、目じるしの ことばだけを 見る。
   */
  await expect(cards.first()).toContainText("初級");
  await expect(cards.first()).toContainText("まいにち");
  await expect(cards.last()).toContainText("上級");
  await expect(cards.last()).toContainText("ひとこと");
});
...
test("ステージIDで 開くと セットを えらぶ 画面に なる", async ({ page }) => {
  ...
  await expect(page.getByText("ことば 24こ ／ 合格 80%")).toBeVisible();
});
