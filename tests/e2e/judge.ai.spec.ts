...
/**
 * 返事の 見かた（judge）— **鍵が あるときの 1往復**
 *
 * `GEMINI_API_KEY` が 渡って いる ときだけ 走る。渡し方は **学習者と同じ道**——
 * 端末（localStorage）に置き、そこから **ブラウザが Google を 直接 呼ぶ**
 *（2026-08-17 から サーバは 通さない）。サーバの環境変数にも ビルドにも 渡さない
 *（OpenNext が バンドルへ 焼き込むため — docs/deploy.md §0.2）。
 *
 * ## このファイルだけ トレースを 取らない
 ...
 *
 * ## `.ai.spec.ts` ＝ ふだんの CI では 走らない
 * 中身が AIの 返答で 変わる ので、対話まわりを 触った PR と 統合・本番への push の
 * ときだけ 走らせる（playwright.config.ts の testIgnore と ci.yml）。
 * 鍵なしの道（毎回 見る ぶん）は `judge.spec.ts` に 残して ある。
 */
