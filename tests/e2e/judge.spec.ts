 ...
 * 渡し方は **学習者と同じ道**——端末（localStorage）に置いて、
 * `/api/meeting/judge` へ 本人の鍵として 送る。サーバの環境変数にも
 * ビルドにも 渡さない（OpenNext が バンドルへ 焼き込むため — docs/deploy.md §0.2）。
 *
 * ## このファイルだけ トレースを 取らない
 * トレースには `/api/meeting/judge` へ 送った 本文（**鍵を含む**）が そのまま 残る。
