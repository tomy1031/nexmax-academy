...
/**
 * 朝礼・夕礼の 司会は 名指しで 呼ぶ（2026-09-15 の 指定）。
 *
 * ただし この 教材は 同じ `◯◯` を **2つの 意味**で 使って いる——
 * `では 次に ◯◯さん` は 名前、`「きのうは ◯◯を しました」の 形で` は 空欄。
 * `fillName` を そのまま 当てると ヒントが「きのうは ソピアを しました」に なる。
 */
describe("呼びかけの 名前だけを 差し込む（朝礼・夕礼）", () => {
  it("◯◯さん が 名前に なる", () => {
    expect(fillCallName("では 次に ◯◯さん、お願いします。", "ソピア")).toBe(
      "では 次に ソピアさん、お願いします。",
    );
  });

  it("**むきだしの ◯◯ は 埋めない**（学習者が 埋める 空欄）", () => {
    expect(fillCallName("「きのうは ◯◯を しました」の 形で お願いします。", "ソピア")).toBe(
      "「きのうは ◯◯を しました」の 形で お願いします。",
    );
    expect(fillCallName("「今、進捗は ◯◯%です」の 形で お願いします。", "ソピア")).toBe(
      "「今、進捗は ◯◯%です」の 形で お願いします。",
    );
  });

  it("名前と 空欄が 同じ 文に あっても、名前だけ 埋める", () => {
    expect(fillCallName("◯◯さん、「きょうは ◯◯を します」の 形で。", "ソピア")).toBe(
      "ソピアさん、「きょうは ◯◯を します」の 形で。",
    );
  });

  /** 名前が まだ 無い 端末（デモ・診断まえ）。「あなたさん」に しない。 */
  it("名前が 無ければ 呼びかけごと 落とす", () => {
    expect(fillCallName("では 次に ◯◯さん、お願いします。", "")).toBe("では 次に お願いします。");
    expect(fillCallName("では 次に ◯◯さん、お願いします。", "   ")).toBe(
      "では 次に お願いします。",
    );
  });

  it("名前が 無くても、空欄は 空欄の まま", () => {
    expect(fillCallName("「きのうは ◯◯を しました」の 形で。", "")).toBe(
      "「きのうは ◯◯を しました」の 形で。",
    );
  });

  it("名前の 印が 無い 文は そのまま", () => {
    expect(fillCallName("ありがとうございます。", "ソピア")).toBe("ありがとうございます。");
  });
});
