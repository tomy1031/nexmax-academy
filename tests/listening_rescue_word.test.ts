...
/**
 * **聞く 前の 画面に 出て いる ことを 承知で 選んだ** あいことば（1件ごとに 理由）。
 *
 * ## 選ぶ 順番（2026-09-22 の 指定）
 * あいことばは **その 授業で おぼえて ほしい ことば**。だから
 * 「画面から 当てられない こと」より **語彙と して 大切な こと**が 先に 来る。
 * 大切な ことばは 見かた（focus）にも 書いて あるのが 当たり前なので、
 * そこに 出る ことは **不合格では ない**——ただし 黙って 増えない ように、
 * 1件ずつ ここに 書き出す。`KNOWN_ESCAPED`（`tests/coverage_walker.test.ts`）と
 * 同じ 運用で、**ここに 無い ものが 出たら 落とす**。1件 直したら 1行 消す。
 */
const ALLOW_ON_SCREEN: Readonly<Record<string, string>> = {
  kaisha_shugyo_keitai_listening:
    "「受託開発」。SES・自社開発と 並ぶ **この 課の 中心の ことば**（ユーザー指定 2026-09-22）。見かた「SES・受託開発・自社開発の「どこで 働くか」に 注目して 聞きましょう。」に そのまま 出る",
  renraku_listening:
    "「検証環境」。台本に 4回 出る **この 連絡の 対象**で、現場で 毎日 使う ことば。せつめいにも 出る",
  soudan_listening:
    "「意見」。相談の 教え どころ（自分の 意見を 先に 言う）その もの。見かたにも 出る",
};
